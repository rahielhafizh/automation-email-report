...
def send_email():
    ...
    fine_year = today.strftime("%Y")
    month_idn_title = get_month_id(month_eng, case="title")
    subject_email = f"Summary Report Penerimaan Denda Aktif | {datetime.now().strftime('%d')} {month_idn_title} ({today.strftime('%H:%M')})"
...
Berikut terlampir Summary Report Penerimaan Denda Aktif As Of {month_idn_title} {fine_year} Pukul {today.strftime('%H:%M')} WIB.
